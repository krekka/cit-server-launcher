...
{ #if part == "bar" }
    <div class="mt-4 w-full rounded-full bg-neutral-700 h-1">
        <div style="width: { (100/filesLength) * filesDownloaded }%;" class="rounded-full bg-green-500 h-1"></div>
    </div>
{ :else if part == "information" }
    <p class="text-neutral-200 text-xs">Завантажено: <span class="underline">{filesDownloaded}/{filesLength} файлів</span></p>
{ /if }